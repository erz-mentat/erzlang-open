...
class CompactParserTests(unittest.TestCase):
    ...
    def test_rule_priority_is_accepted_and_formatted_canonically(self) -> None:
        source = (
            'rule{id:"r-priority",then:[{kind:"act",params:{rule_id:"r-priority"}}],'
            'priority:100,when:["event_type_present"]}'
        )

        parsed = parse_compact(source)
        self.assertEqual(parsed[0]["fields"]["priority"], 100)

        formatted = parse_and_format_compact(source)
        self.assertEqual(
            formatted,
            'rule{id:"r-priority",when:["event_type_present"],then:[{kind:"act",params:{rule_id:"r-priority"}}],priority:100}\n',
        )

    def test_rule_priority_rejects_non_integer_values(self) -> None:
        for source in (
            'rule{id:"r1",when:["event_type_present"],then:[{kind:"act",params:{rule_id:"r1"}}],priority:true}',
            'rule{id:"r1",when:["event_type_present"],then:[{kind:"act",params:{rule_id:"r1"}}],priority:1.5}',
            'rule{id:"r1",when:["event_type_present"],then:[{kind:"act",params:{rule_id:"r1"}}],priority:"high"}',
        ):
            with self.subTest(source=source):
                with self.assertRaisesRegex(CompactValidationError, "priority"):
...
class CliCommandTests(unittest.TestCase):
    ...
    def test_eval_command_emits_deterministic_envelope(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            program_file = Path(tmp_dir) / "program.erz"
            event_file = Path(tmp_dir) / "event.json"

            program_file.write_text(
                'rule{id:"r1",when:["event_type_present"],then:[{kind:"act",params:{channel:"ops"}}]}',
                encoding="utf-8",
            )
            event_file.write_text(
                '{"type":"ingest","payload":{"source":"telegram"}}',
                encoding="utf-8",
            )

            result = self._run_cli("eval", str(program_file), "--input", str(event_file))
            self.assertEqual(result.returncode, 0)
            ...
            self.assertEqual(
                envelope["actions"],
                [{"kind": "act", "params": {"channel": "ops"}}],
            )
            self.assertEqual(envelope["trace"], [{"rule_id": "r1", "matched_clauses": ["event_type_present"], "score": 1.0}])

    def test_eval_command_returns_runtime_error_envelope(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            program_file = Path(tmp_dir) / "program.erz"
            event_file = Path(tmp_dir) / "event.json"
            ...
            event_file.write_text(
                '{"type":"ingest","payload":"not-a-mapping"}',
                encoding="utf-8",
            )
    ...
    def test_eval_command_fails_on_invalid_input_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            program_file = Path(tmp_dir) / "program.erz"
            event_file = Path(tmp_dir) / "event.json"
            ...
            event_file.write_text('{"type":"ingest",', encoding="utf-8")
